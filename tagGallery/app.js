document.addEventListener('DOMContentLoaded', () => {
	...
	function addNewImageLi(newURL){
		...
		li.innerHTML = `<select>${generateSelectHTML()}</select><img src= ${newURL} width = 300>`;
	...
	function generateSelectHTML(){
		...
		for (let i=0; i<tags.length; i++){
			newContent = newContent + `<option value=${tags[i].textContent}>${tags[i].textContent}</option>`;
		}
		return newContent;
	}
	...
	for(let i = 0; i<lis.length; i++){
		...
		lis[i].querySelector('select').innerHTML = newContent;
	}
